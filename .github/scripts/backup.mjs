...
async function main() {
  ...
  for (const [setor, cfg] of Object.entries(cfgs || {})) {
    ...
    // ★ DIA DE REFERÊNCIA: SEMPRE o dia ANTERIOR ao backup. Backup rodando
    // na quarta = lista de TERÇA. A pasta e o rótulo do backup usam esse dia.
    const DIA_NOMES_FULL = ["DOMINGO","SEGUNDA","TERÇA","QUARTA","QUINTA","SEXTA","SÁBADO"];
    const ontemD = new Date(agora.getTime() - 24 * 60 * 60 * 1000);
    const bloco = [{ nome: DIA_NOMES_FULL[ontemD.getUTCDay()], data: slotDoDia(ontemD) }];
    const dataRef = bloco[0].data;               // pasta nomeada pelo dia anterior
    const dataRefFim = slotHoje;                 // fotos vão ATÉ hoje (horário do backup)
    const periodoLabel = `${bloco[0].nome} (${bloco[0].data})`;
    const datasDoBloco = new Set([bloco[0].data, slotHoje]);

    // FOTOS: entram TODAS as fotos pendentes desde o último backup (o
    // backup anterior já apagou as que ele levou, então o que sobrou é
    // exatamente "depois do último backup") ATÉ o horário deste backup.
    // Depois disso, reseta (apaga do site) — e o próximo ciclo recomeça.
